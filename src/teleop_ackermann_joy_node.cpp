...
public:
  // Constructor
  TeleopAckermannJoy()
  : Node("teleop_ackermann_joy_node", rclcpp::NodeOptions().enable_logger_service(true))
  {
    ...
    activation_subscription_ = this->create_subscription<std_msgs::msg::Bool>(
      "/activate", 10,
      std::bind(&TeleopAckermannJoy::activation_callback, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(
      this->get_logger(), "Require Enable Button: %s", require_enable_button_ ? "true" : "false");
    ...
    RCLCPP_INFO(
      this->get_logger(), "Steering Axis: %d, Scale: %.2f, Inverted: %s", axis_steering_,
      scale_steering_, invert_steering_ ? "true" : "false");
  ...
  void activation_callback(const std_msgs::msg::Bool::SharedPtr msg)
  {
    is_active_ = msg->data;
    RCLCPP_INFO(
      this->get_logger(), "Teleop Ackermann controller %s",
...
private:
  ...
  void joy_callback(const sensor_msgs::msg::Joy::SharedPtr msg)
  {
    ...
    RCLCPP_DEBUG(
      this->get_logger(), "Joy message - Axes count: %zu, Buttons count: %zu", msg->axes.size(),
      msg->buttons.size());
    ...
    for (size_t i = 0; i < msg->axes.size(); ++i) {
      if (std::abs(msg->axes[i]) > 0.01) {  // Only log non-zero axes
    ...
    if (is_active_ && require_enable_button_) {
      ...
      if (msg->buttons.size() > enable_button_) {
        ...
        RCLCPP_DEBUG(
          this->get_logger(), "Enable button %d state: %s", enable_button_,
          should_process_commands ? "PRESSED" : "NOT PRESSED");
      } else {
        should_process_commands = false;
        RCLCPP_WARN(
          this->get_logger(), "Enable button %d not available (only %zu buttons)", enable_button_,
          msg->buttons.size());
      }
    } else if (is_active_) {
      // No enable button required but node is active - always process commands
      RCLCPP_DEBUG(
        this->get_logger(), "Enable button disabled - processing commands (node active)");
    } else {
    ...
    if (should_process_commands) {
      ...
      if (use_trigger_mode_) {
        ...
        if (msg->axes.size() > axis_forward_trigger_) {
          ...
          RCLCPP_DEBUG(
            this->get_logger(), "Forward trigger (axis %d): %.3f", axis_forward_trigger_,
            forward_throttle);
        } else {
          RCLCPP_WARN(
            this->get_logger(), "Forward trigger axis %d not available (only %zu axes)",
        ...
        if (msg->axes.size() > axis_reverse_trigger_) {
          ...
          RCLCPP_DEBUG(
            this->get_logger(), "Reverse trigger (axis %d): %.3f", axis_reverse_trigger_,
            reverse_throttle);
        } else {
          RCLCPP_WARN(
            this->get_logger(), "Reverse trigger axis %d not available (only %zu axes)",
        ...
        RCLCPP_DEBUG(
          this->get_logger(), "Combined throttle: %.3f (forward: %.3f, reverse: %.3f)",
        ...
        if (msg->axes.size() > axis_speed_) {
          ...
          RCLCPP_DEBUG(
            this->get_logger(), "Speed axis %d raw value: %.3f (dead zone: %.3f)", axis_speed_,
          ...
          ackermann_msg->drive.speed = 0.0;
          RCLCPP_WARN(
            this->get_logger(), "Speed axis %d not available (only %zu axes)", axis_speed_,
            msg->axes.size());
        }
      }
      ...
      if (msg->axes.size() > axis_steering_) {
        ...
        RCLCPP_DEBUG(
          this->get_logger(), "Steering axis %d raw value: %.3f (dead zone: %.3f)", axis_steering_,
          raw_steering, axis_dead_zone_);
        ...
        if (std::abs(raw_steering) > axis_dead_zone_) {
          ...
          RCLCPP_DEBUG(
            this->get_logger(),
            "Steering calculated: %.3f (raw: %.3f, inverted: %s, * scale: %.3f)",
            ackermann_msg->drive.steering_angle, raw_steering, invert_steering_ ? "true" : "false",
            scale_steering_);
        } else {
          ackermann_msg->drive.steering_angle = 0.0;
        ...
        ackermann_msg->drive.steering_angle = 0.0;
        RCLCPP_WARN(
          this->get_logger(), "Steering axis %d not available (only %zu axes)", axis_steering_,
          msg->axes.size());
      }

      RCLCPP_DEBUG(
        this->get_logger(), "Publishing Ackermann command - Speed: %.3f, Steering: %.3f",
